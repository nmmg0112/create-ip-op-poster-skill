...
这份成功案例仍然只负责证明“最终 Prompt 应该有多完整”。人物素材必须在它之前确认，且不再额外生成排布确认稿。

### 用于模式 A

复用本例的内容完整度和信息组织方法，再按 `references/prompt-template.md` 转成模式 A 专用 Prompt。原案例中的强保真措辞不能原样沿用；必须逐项列出允许模型参考并重绘的 `P/C/L/T`，并向用户说明人物、截图、Logo 和中文可能被重新绘制，不能声称原样保留。任一素材必须原样保留时，改选模式 B。

### 用于模式 B

先从整份 Prompt 中提取主题、场景、艺术构图、材质、光影、前中后景、装饰语言和保护图层预留关系，生成一张完整的 PNG、WebP 或 JPEG 位图底图。底图不得由 SVG、HTML、PPT、Canvas、Sharp、固定色块或网格生成。

位图底图生成后，再把已确认的透明人物总图或独立抠图、六张完整案例截图、原始 Logo 和准确中文作为保护图层覆回。最终人物可以按本例整组呈现，也可以在其他项目中按玩法拆分；本例的左文右人、季节、配色和人物主次不得固化为通用规则。
