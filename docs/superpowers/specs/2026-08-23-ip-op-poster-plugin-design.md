...
首版只解决一件事：让 ChatGPT 和 Codex 能稳定调用与现有网页版 `create-ip-op-poster` Skill 功能等价的 IP/OP 海报工作流。首版不接 MCP 服务，不登录第三方账号，不收集用户数据，也不执行外部写操作。

## 功能等价原则

现有网页版 Skill 是唯一功能基准。Plugin 不得主动删除、合并或弱化以下能力：Brief 解析、单人/多人玩法分类、视觉案例检索、视觉导演、版式选择、信息密度判断、人物/动物抠图确认、人物排布确认、完整成图 Prompt 确认、受保护素材规则、新手模式、跨平台交接和最终 QA。

“功能等价”指相同输入应进入相同阶段、读取同类参考、执行相同确认 Gate、产出同结构的方向与 Prompt，并执行相同的素材保护和验收规则。它不承诺生成模型输出像素完全一致，也不允许 Skill 假装宿主平台拥有其实际没有的保真抠图或分层合成工具。

构建时必须生成一份逐项功能对照清单，并通过自动校验确认网页版 Skill 中的必需文件、关键规则、启动语和回退行为都已进入 Plugin。任何因公开分发边界而不能原样携带的资源，必须用功能等价的公开替代物承接，不能静默删除。
...
公开包不带授权状态不明的行业海报原图、联系表或其他第三方案例图片。视觉案例库改成文字化设计语法和自制的匿名版式示意图，保留排版家族、信息密度、人物组织方式、容器关系和适用条件。用户需要参考具体海报时，在当前任务中自行上传有权使用的参考图。替换只改变案例资源的公开表达，不改变案例检索和视觉决策能力。
...
先运行 Plugin 结构校验、Skill 校验和网页版/Plugin 功能等价校验，再做本地安装测试。行为测试至少覆盖：单达人、多人矩阵、没有案例图的新手输入、平台缺少保真图像能力，以及截图或人物保护规则被要求跳过的负向场景。

完成标准：

- Plugin 清单通过官方结构校验。
- 内含 Skill 通过原有静态校验。
- 功能对照清单中的必需能力全部为 `PASS`，不存在未解释的缺失项。
